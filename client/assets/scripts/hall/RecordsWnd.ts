...
function snapToSettle(snap: any): ResultSettleInfo {
  ...
  const rows = seats.map((s: any) => ({
    seat: Number(s.seat),
    name: String(s.userName || `座位${s.seat}`),
    score: Number(s.scoreDelta) || 0,
...
export class RecordsWnd {
  static async show(parent: Node): Promise<Node> {
    ...
    root.addChild(panel);
    panel.layer = layer;
    panel.addComponent(UITransform).setContentSize(780, 560);
    panel.setPosition(0, 10, 0);
    const pg = panel.addComponent(Graphics);
    pg.fillColor = new Color(22, 30, 42, 250);
    pg.roundRect(-390, -280, 780, 560, 20);
    pg.fill();
    ...
    void loadSpriteFrame('weihai/ui/settle/glass_bg').then((sf) => {
      if (sf && panel.isValid) {
        psp.spriteFrame = sf;
        const tw = sf.originalSize?.width || 780;
        const th = sf.originalSize?.height || 560;
        const s = Math.min(820 / tw, 580 / th);
    ...
    titleBar.layer = layer;
    titleBar.setPosition(0, 230, 0);
    titleBar.addComponent(UITransform).setContentSize(700, 56);
    const tbs = titleBar.addComponent(Sprite);
    tbs.sizeMode = Sprite.SizeMode.CUSTOM;
    void loadSpriteFrame('weihai/ui/settle/title_bar').then((sf) => {
      if (sf && titleBar.isValid) {
        tbs.spriteFrame = sf;
        const tw = sf.originalSize?.width || 700;
        const th = sf.originalSize?.height || 56;
        const s = Math.min(700 / tw, 56 / th);
        titleBar.getComponent(UITransform)!.setContentSize(tw * s, th * s);
      }
    });
    ...
    const listHost = new Node('list');
    panel.addChild(listHost);
    listHost.layer = layer;
    listHost.setPosition(0, 20, 0);
    listHost.addComponent(UITransform).setContentSize(700, 380);

    const tip = new Node('tip');
    listHost.addChild(tip);
    tip.layer = layer;
    tip.addComponent(UITransform).setContentSize(640, 40);
    const tipLab = tip.addComponent(Label);
    styleLabel(tipLab, 20);
    tipLab.string = '加载中…';
    tipLab.color = new Color(230, 220, 190, 255);

    const close = RecordsWnd.mkBtn(panel, layer, 0, -240, 160, 48, '关闭', () => {
      RecordsWnd.hide(parent);
    });

    void RecordsWnd.loadList(listHost, tipLab, root);
  ...
  private static async loadList(listHost: Node, tipLab: Label, root: Node) {
    try {
      const msg = await NetBus.ins.getRecords(1, 12);
      if (msg.cmd === 'error') {
      ...
      for (let i = 0; i < list.length; i++) {
        await RecordsWnd.addCard(listHost, list[i], i, root);
      }
    } catch (e) {
  ...
  private static async addCard(host: Node, rec: RecordItem, index: number, root: Node) {
    const layer = host.layer;
    const win = (rec.scoreDelta || 0) > 0;
    const lose = (rec.scoreDelta || 0) < 0;
    const cardH = 72;
    const gap = 8;
    const top = 160;
    ...
    card.addComponent(UITransform).setContentSize(680, cardH);
    ...
    bg.roundRect(-340, -cardH / 2, 680, cardH, 12);
    bg.fill();
    bg.strokeColor = win
    ...
    card.addChild(badge);
    badge.layer = layer;
    badge.setPosition(-290, 0, 0);
    badge.addComponent(UITransform).setContentSize(56, 56);
    ...
    void loadSpriteFrame(badgePath).then((sf) => {
      if (sf && badge.isValid) {
        bsp.spriteFrame = sf;
        const tw = sf.originalSize?.width || 56;
        const th = sf.originalSize?.height || 56;
        const s = Math.min(52 / tw, 52 / th);
    ...
    mid.addComponent(UITransform).setContentSize(360, 28);
    const midLab = mid.addComponent(Label);
    styleLabel(midLab, 20);
    ...
    card.addChild(sub);
    sub.layer = layer;
    sub.setPosition(-40, -16, 0);
    sub.addComponent(UITransform).setContentSize(360, 22);
    const subLab = sub.addComponent(Label);
    styleLabel(subLab, 16);
    ...
    card.addChild(score);
    score.layer = layer;
    score.setPosition(270, 0, 0);
    score.addComponent(UITransform).setContentSize(120, 40);
    const scoreLab = score.addComponent(Label);
    styleLabel(scoreLab, 28);
  ...
  private static async openDetail(root: Node, rec: RecordItem) {
    ...
    try {
      ...
      const isDraw = (settle.reason || '').includes('huang');
      const title = isDraw ? '平局' : (myScore > 0 ? '胜利' : '失败');
      await SettleWnd.show(parent, {
        title,
        sub: settle.detail || '',
        roomId: settle.roomId ?? rec.roomId,
        primaryLabel: '关闭',
        onPrimary: () => SettleWnd.hide(parent),
        settle,
      });
    } catch (e) {
